...
class Game:
    def __init__(self,restart,model,cam,im_shape,speed,size=1):
    ...
    def over(self):
        while 1:
            ...
            self.screen.blit(txts,txtrect)
            pg.display.update()
            self.make_button((153,300,100,50),'Restart', [(255,255,255),(150,150,150)], action = lambda:self.restart())
            if self.hover ==True:
    ...
    def loop(self):
        ...
        while self.game_over != True:
            ...
            CLS = {0:'Top',1:'Right',2:'Bottom',3:'Left'}
            print(CLS[int(out[0])],)
...
class SnakeGame:
    def __init__(self,model,cam,im_shape):
        self.cam = cam
    ...
    def start(self):
        pg.init()
        self.smanu = StartMenu(self.start_game)
        self.smanu.mainloop()

    def start_game(self,speed,size):
        self.game = Game(self.restart,self.__model_class,self.cam,self.im_shape,speed,size)
        self.game.loop()
