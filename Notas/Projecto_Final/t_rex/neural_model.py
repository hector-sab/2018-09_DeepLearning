...
class Model:
	...
	def __model(self):
		...
		self.conv4 = tf.layers.conv2d(inputs=self.pool3,filters=64,kernel_size=3,activation=tf.nn.relu)
		self.pool4 = tf.layers.max_pooling2d(self.conv4,pool_size=2,strides=2)

		#self.conv5 = tf.layers.conv2d(inputs=self.pool4,filters=64,kernel_size=3,activation=tf.nn.relu)
		#self.pool5 = tf.layers.max_pooling2d(self.conv5,pool_size=2,strides=2)

		shape = self.pool4.get_shape()
		self.flat1 = tf.reshape(self.pool4,[-1,shape[1].value*shape[2].value*shape[3].value])
